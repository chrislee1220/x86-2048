...
function callCpuTemperature() {
	return new Promise(function(resolve) {
		var xhr = new XMLHttpRequest();

		xhr.open('GET', '/cgi-bin/cpu-temperature?_=' + Date.now(), true);
		xhr.onload = function() {
			resolve(xhr.status === 200 ? xhr.responseText.trim() : null);
		};
...
return baseclass.extend({
	...
	load: function() {
		return Promise.all([
			...
			uci.load('system'),
			L.resolveDefault(callCpuTemperature(), null)
		]);
	},
